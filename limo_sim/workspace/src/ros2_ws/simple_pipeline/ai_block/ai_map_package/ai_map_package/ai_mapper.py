...
class Mapper(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('roi_width_far_m', 2.20)
        ...
        if self.color_flag not in self.COLORS:
            raise ValueError(
                f'Unsupported color {self.color_flag!r}; use {", ".join(self.COLORS)}'
            )
        ...
        costmap_topic = f'/limo/ai_map_package/costmap/costmap_grid_{self.color_suffix}'
        map_topic = f'/limo/ai_map_package/mapper/map_paper_{self.color_suffix}'

        self.filtered_publisher = self.create_publisher(OccupancyGrid, map_topic, 10)
        self.roi_debug_publisher = self.create_publisher(
            Image, f'/limo/ai_map_package/mapper/roi_debug_{self.color_suffix}', 10
        )
    ...
    def timer_callback(self):
        ...
        try:
            ...
            if self.cached_shape != (ch, cw):
                ...
                max_allowed_y = allowed_width / 2.0
