"""Online navigation map built from the static, CV and lidar inputs."""
...
class OnlineNavMap(Node):
    """Publish one online map combining SLAM, CV costs and lidar endpoints.

    This node is deliberately independent from the offline ``NavMap`` node.
    The static map supplies the output geometry and initial cell values. The
    latest CV grid and laser scan are transformed into that geometry and merged
    without allowing free source cells to erase existing obstacles.
    """

    def __init__(self):
        super().__init__('online_map')

        # Input, output and map-frame configuration.
        self.declare_parameter('global_frame', 'map')
        self.declare_parameter('static_map_topic', '/map')
        self.declare_parameter(
            'cv_grid_topic',
            '/limo/map_package/online/metric_bev/cost_grid_combined',
        )
        ...
        # Transient-local map QoS makes the most recent map available to late
        # subscribers. LaserScan retains its standard sensor-data QoS.
        map_qos = QoSProfile(
        ...
        # Callbacks cache only the latest inputs; the timer builds each output.
        ...
        self.get_logger().info(
            f'Online mapping: static={self.static_map_topic}, '
            f'cv={self.cv_grid_topic}, laser={self.scan_topic} -> '
            f'combined={self.output_topic}'
        )

    def static_map_callback(self, msg: OccupancyGrid) -> None:
        """Cache the map used as output geometry and combined-map base."""
        self.static_map = msg

    def cv_grid_callback(self, msg: OccupancyGrid) -> None:
        """Cache the latest online computer-vision cost grid."""
        self.cv_grid = msg
    ...
    def _lookup_transform(self, source_frame: str, stamp):
        ...
        try:
            # Prefer the transform at acquisition time.
            ...
            # Simulated sensor timestamps can fall between two TF updates. In
            # that case use the newest transform rather than dropping the layer.
            return self.tf_buffer.lookup_transform(
                self.global_frame,
                source_frame,
                Time(),
    ...
    def _world_to_output_cells(self, x: np.ndarray, y: np.ndarray):
        """Convert global coordinates into valid output rows and columns."""
        ...
        # Apply the inverse map-origin transform. OccupancyGrid x addresses a
        # column and y addresses a row in the flattened row-major array.
    ...
    def _overlay_cv_grid(self, combined: np.ndarray) -> None:
        """Project all positive cells from the online CV grid."""
        msg = self.cv_grid
        ...
        # Convert CV cell centers into metric coordinates in the grid frame.
        ...
        # Several source cells can land in one output cell; retain the maximum.
        np.maximum.at(combined.ravel(), flat_indices, costs)

    def _overlay_scan(self, combined: np.ndarray) -> None:
        """Project valid endpoints from the latest lidar scan."""
        msg = self.scan
        ...
        # Convert valid polar samples to Cartesian sensor-frame endpoints.
        ...
        combined[rows, cols] = np.maximum(
            combined[rows, cols],
            self.lidar_cost,
        )

    def publish_combined_map(self) -> None:
        """Merge the latest available inputs and publish the online map."""
...
def main(args=None):
    """Initialize ROS, run the online node and shut it down cleanly."""
