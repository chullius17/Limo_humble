    ...
    def __init__(self):
        ...
        if self.publish_debug:
            self.roi_debug_pub = self.create_publisher(
                Image,
                f'/limo/map_package/{self.topic_namespace}/roi_debug',
                debug_image_qos,
            )
            self.combined_debug_pub = self.create_publisher(
                Image,
                f'/limo/map_package/{self.topic_namespace}/debug_combined',
                debug_image_qos,
            )
            self.obstacle_debug_pub = self.create_publisher(
                Image,
                f'/limo/map_package/{self.topic_namespace}/debug_binary_obstacles',
                debug_image_qos,
            )
            self.street_debug_pub = self.create_publisher(
                Image,
                f'/limo/map_package/{self.topic_namespace}/debug_binary_street',
                debug_image_qos,
            )

        self.combined_costmap_pub = self.create_publisher(
            OccupancyGrid,
            f'/limo/map_package/{self.topic_namespace}/cost_grid_combined',
            map_qos_profile,
        )
        self.obstacle_costmap_pub = self.create_publisher(
            OccupancyGrid,
            f'/limo/map_package/{self.topic_namespace}/cost_grid_binary_obstacles',
            map_qos_profile,
        )
        self.street_costmap_pub = self.create_publisher(
            OccupancyGrid,
            f'/limo/map_package/{self.topic_namespace}/cost_grid_binary_street',
            map_qos_profile,
        )
